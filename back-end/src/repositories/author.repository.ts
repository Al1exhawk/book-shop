...
export class AuthorRepository {
  ...
  ): Promise<{ pages: number; authors: AuthorDocument[] }> {
    const amount = await this.authorModel.find().countDocuments();
  ...
  async deleteItemFromAuthors(id: string) {
    const updDatedItems = await this.authorModel.updateMany(
      { items: id },
      { $pull: { items: id } },
    );
  ...
  async findByRegExp(searchString: string): Promise<AuthorDocument[]> {
    ...
    const authorItems: AuthorDocument[] = await this.authorModel.find({
